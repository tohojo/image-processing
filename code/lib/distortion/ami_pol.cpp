...
/**
 * \fn long double ami_horner(long double *pol,int degree,long double x,long double *fp)
 * \brief function to evaluate a polynomial and its derivate using Horner Algorithm
 * \return polynomial evaluation
 * \author Luis Alvarez
*/
long double ami_horner(
long double *pol /** coefficient polynomial vector pol[0]+pol[1]*x+pol[2]*x^2+... */,
...
/**
 * \fn long double ami_root_bisection(long double *pol,int degree,long double a,long double b,long double TOL)
 * \brief function to compute the polynomial root in an interval. We use a combination of bisection and Newton-Raphson tecniques
...
{
  long double a2=a,b2=b,fa,fb,c,c0,fc,fp;
  
  fp=0.0; /*Added by Luis Gomez*/
...
/**
 * \fn int ami_polynomial_root(double *pol, int degree, double *root_r, double *root_i)
 * \brief function to estimate real polynomial roots. We use a recursive procedure computing the derivative polynomial roots.
...
double *pol /** coefficient polynomial vector pol[degree]+pol[degree-1]*x+pol[degree-2]*x^2+... FOR HISTORICAL REASONS THE
                ORDER POLYNOMIAL COEFICIENT IS GIVEN IS DIFFERENT THAT IN THE OTHERS POLYNOMIAL FUNCTIONS */,
int degree /** polynomial degree */,
double *root_r /** output real component of polynomial roots */,
double *root_i /** output complex component of polynomial roots. Since this function only real roots this complex part is fitted to 0.
                 FOR HISTORICAL REASONS WE KEEP THIS COMPLEX PART IN THE FUNCTION*/)
{
  ...
  fp=0.0;a=b=fa=fb=max=n_factor=0.0; /*Added by Luis Gomez*/
  ...
  /* WE NORMALIZE POLINOMIAL COEFFICIENTS TO AVOID POLYNOMIAL EVALUATION IN LARGE NUMBERS*/
  n_factor=0;
  for(k=0;k<N;k++){
    if(fabs(pol2[k])>10.){
  ...
  /* WE COMPUTE THE DERIVATIVE POLINOMIAL ROOTS IN A RECURSIVE WAY */
	for(k=N-2;k>=0;k--){
	  /* we compute polynomial derivative coefficients */
		for(l=0;l<=(N-k);l++){ ap[l]=pol2[l+k]*(f[k+l]/f[l]); }
		...
		for(l=1;l<=(N-k);l++){  p_aux[l]=ami_root_bisection(ap,N-k,p[l-1],p[l],TOL); }
			}
	/* WE STORE THE ROOTS */
  ...
  /* we free the memory */
  free(p); free(ap); free(f); free(pol2); free(p_aux);
